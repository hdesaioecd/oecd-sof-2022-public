...
labels = c(".PC1", ".PC2")
agg <- NULL
for (i in names(all.pca)[-6]){
    temp <- as.data.frame(all.pca[[i]]$`2017`$x[,1:2])
    names(temp) = paste0(i, labels)
    agg = dplyr::bind_cols(agg, temp)
}
agg = data.frame(iso3c = all.pca[[i]]$`2017`$iso3c, agg)
agg = agg %>% dplyr::filter(iso3c %in% fragile$iso3c)
dim.pca = my.pca(agg, "dimens fragile", max(sfr.time.series$year))
agg = bind_rows(agg, .id = "year")
num.clusters <-NbClust(agg[,-c(1:2)], distance = "euclidean", min.nc=2, max.nc=10, 
             method = "ward.D", index = "all")
num.clusters$n = length(unique(num.clusters$Best.partition))

# calculate a pca
pca <- my.pca(agg[,-c(1)], "fragile", max(sfr.time.series$year))
